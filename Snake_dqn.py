...
class DQNAgent:
    ...
    def __init__(self, state_size, action_size):
        ...
        #Set hyper parameters for the DQN
    ...
    def build_model(self):
        model = Sequential() #Create linear stack of layers
        model.add(Dense(128, input_dim=self.state_size, activation='relu', #Done
                        kernel_initializer='he_uniform')) #Fully connected layer with 128 nodes and ReLU activation function
        model.add(Dense(self.action_size, activation='linear',
                        kernel_initializer='he_uniform')) #Output layer with linear activation function.
        model.summary()
        model.compile(loss='mse', optimizer=Adam(lr=self.learning_rate)) #Build the model and train it using a mean square error loss function and the Adam optimizer (SGD variant)
        return model
    ...
    def get_action(self, state):
        if np.random.rand() <= self.epsilon: #If sample from [0,1] <= epsilon:
            action = random.randrange(self.action_size) #Sample uniformly over the action space
        else:
            q_value = self.model.predict(state) #Get Q(s,a) from the network for given state
            action = np.argmax(q_value) #Return the action that maximizes Q
        return action
    ...
    def train_model(self):
        ...
        for i in range(self.batch_size): #For every batch
            if done[i]: #If iteration i corresponds to the last iteration of its episode
                target[i][action[i]] = reward[i] #Store the reward for the action taken
            else:
                target[i][action[i]] = reward[i] + self.discount_factor * (
                    np.amax(target_val[i])) #Store reward + discounted value using target network
...
if __name__ == "__main__":
    ...
    env = SnakeEnv() #Generate Cartpole-v0 environment object from the gym library
